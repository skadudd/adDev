...
path = '../kwd_combiner/query/attr'
today = date.today()
#df = pd.read_csv('/Users/maketing/adDev/kwd_combiner/query/기존쿼리.csv')
#kwd = df['연관 검색어'][0]

input_file = "../kwd_combiner/query/attr"
...
print(data_combine)
# regexed = regex_do(data_combine)
data_combine2 = data_combine.drop_duplicates(['keyword'],keep='first')
#print(len(df2))
print(len(data_combine2))
# regexed.to_csv(Path(path, f'{today.year}_{today.month}_필터된키워드2.csv'), index=False)
data_combine.to_csv(Path(path, f'{today.year}_{today.month}_필터된키워드2.csv'), index=False)